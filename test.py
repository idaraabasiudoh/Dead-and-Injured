...
class DeadAndInjuredGame:
    ...
    def show_guide(self):
        ...
        guide_text = """
        ...  # (The guide text remains the same)
    ...
    def setup_game_panel(self):
        ...
        """
        guide_panel = Label(self.root, text="\t\t        WELCOME TO DEAD AND INJURED\t\t\t", bg="#1d1e1f",
                            font=('FixedSys', 18, 'bold'))
        guide_panel.pack()

        version_label = Label(self.root, text="\t\t          Version 1.0.0 (Hard)\t\t\t", bg="#1d1e1f", fg="red",
    ...
    def setup_history_frame(self):
        ...
        self.history_text = Text(history_frame_2, bg="#131313", fg="white", wrap="none", font=('FixedSys', 12),
                                 height=10, width=50)
    ...
    def configure_gui_styles(self):
        ...
        """
        style = ttk.Style()
        style.configure("Rounded.TEntry", borderwidth=0, bordercolor="blue", relief="flat", background="blue",
                        padding=(5, 5), width=200)
    ...
    def check_input(self, number):
        ...
        if len(number) != 5:
            self.result_label.config(
                text=f"Error: Number must be 5 digit\n i.e XXXXX\nTry Again")
            self.style.configure("Rounded.TFrame", background="red", relief="groove")
            return False
        ...
        for digit in number:
            if digit not in new_list:
                ...
                self.result_label.config(
                    text=f"Error: Number must have 5 different digits\nExample: '12345' is allowed but '12344' is invalid.\nTry Again")
                self.style.configure("Rounded.TFrame", background="red", relief="groove")
                return False
    ...
    def handle_successful_response(self):
        ...
        self.result_label.config(
            text=f"5 Dead --------- 0 Injured\nYou won\n\nAttempts: {self.trials_used}\nElapsed Time: {(timer_seconds // 60):.0f} mins {(timer_seconds % 60):.0f} seconds")
