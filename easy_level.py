...
class DeadAndInjuredGame:
    def __init__(self):
        """
        Initializes the Dead and Injured game GUI sets up initial configurations.
        """
        self.root = tk.Tk()
    ...
    def show_guide(self):
        ...
        guide_text = """
        INTRODUCTION
        Welcome to the "Dead and Injured" game version 1.0.0! This is a number guessing game where your goal is to figure out a secret three-digit number. As you make guesses, the game provides feedback on the number of correct digits in the correct position (Dead) and the number
        of correct digits in the wrong position (Injured). Use this information to refine your guesses and win the game!

        HOW TO PLAY
        1. Getting Started:
        Run the application provided to launch the game interface. Upon starting the game, a random three-digit number will be generated for you to guess. This number is your target, and the game will provide feedback based on your guesses.

        2. Making a Guess:
        Enter your three-digit guess into the input field labeled "Enter your guess below." Click the "Done" button to submit your guess and receive feedback.
        ...
        4. Winning:
        If you correctly guess all three digits in the correct positions, you win! The game will display your winning message along with the number of attempts made and the elapsed time.
        ...
        ERROR MESSAGES
        1. Invalid Input:
        If you enter a guess that is not a three-digit number, the game will display an error message. Example: "Error: Number must be 3 digits, i.e., XXX. Try Again."

        2. Repeated Digits:
        If your guess contains repeated digits, the game will prompt you to enter a number with three different digits. Example: "Error: Number must have 3 different digits. Example: '123' is allowed, but '122' is invalid. Try Again."
    ...
    def initialize_variables(self):
        ...
        self.end_time = 0
        self.three_digit_number = random.sample(range(10), 3)
        self.initial_guess = ''.join(map(str, self.three_digit_number))
        self.final_answer = '{:03}'.format(self.initial_guess)
    ...
    def setup_game_panel(self):
        ...
        version_label = Label(self.root, text="\t\t          Version 1.0.0 (Easy)\t\t\t",  bg="#1d1e1f", fg="red", font=('FixedSys', 10, 'bold'))
        version_label.pack()
    ...
    def setup_entry_widget(self):
        ...
        """
        label_name = Label(self.root, text="Enter your 3-digit guess below",  bg="#1d1e1f")
        label_name.pack()
    ...
    def check_input(self, number):
        ...
        """
        if len(number) != 3:
            self.result_label.config(text=f"Error: Number must be 3 digit\n i.e XXX\nTry Again")
            self.style.configure("Rounded.TFrame", background="red", relief="groove")
            return False
        ...
        for digit in number:
            if digit not in new_list:
                ...
                self.result_label.config(text=f"Error: Number must have 3 different digits\nExample: '123' is allowed but '122' is invalid.\nTry Again")
                self.style.configure("Rounded.TFrame", background="red", relief="groove")
                return False
